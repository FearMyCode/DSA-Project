...
public class ShallowsTester {
	...
	private static class TestCase implements Testable {
		...
		public static TestCase fromFile(File file) throws FileNotFoundException {
			...
			test.ports = scanner.nextInt();
			test.lanes = new Shallows.Lane[scanner.nextInt()];
			test.origin = scanner.nextInt();

			for (int i = 0; i < test.lanes.length; i++) {
				Shallows.Lane lane = new Shallows.Lane();
				lane.depart = scanner.nextInt();
				lane.arrive = scanner.nextInt();
				lane.depth = scanner.nextInt();
				test.lanes[i] = lane;
			}
		...
		public boolean run(boolean earlyExit) {
			try {
				int[] received = runTimed(() -> {
					Shallows shallows = new ShallowsImpl();
					return shallows.maximumDraughts(ports, lanes, origin);
				});
	...
	public static void main(String[] args) {
		Testable testable;
		try {
			testable = TestTree.fromFile(
					new File("C:\\Users\\mcaly\\eclipse-workspace\\CITS2200_Project_2022-release\\Shallows\\testdata"));
		} catch (Throwable e) {
		...
		testable.buildReport(builder, "Testing Shallows", 0);
