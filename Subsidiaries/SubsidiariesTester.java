...
public class SubsidiariesTester {
	private static final long TIME_LIMIT_SECONDS = 2;
	private static final String INDENT = "    ";

	private enum TestState {
		PASS, FAIL, ERROR, TIME_LIMIT_EXCEEDED, UNKNOWN,
	}
	...
	private static class TestCase implements Testable {
		private int[] owners;
		private Subsidiaries.Query[] queries;
		...
		public static TestCase fromFile(File file) throws FileNotFoundException {
			...
			test.owners = new int[scanner.nextInt()];
			for (int i = 0; i < test.owners.length; i++) {
				test.owners[i] = scanner.nextInt();
			}

			test.queries = new Subsidiaries.Query[scanner.nextInt()];
			for (int i = 0; i < test.queries.length; i++) {
				Subsidiaries.Query query = new Subsidiaries.Query();
				query.payer = scanner.nextInt();
				query.payee = scanner.nextInt();
		...
		public boolean run(boolean earlyExit) {
			try {
				int[] received = runTimed(() -> {
					Subsidiaries subsidiaries = new SubsidiariesImpl();
					return subsidiaries.sharedOwners(owners, queries);
				});
	...
	public static <T> T runTimed(Supplier<T> supplier)
	...
	public static void main(String[] args) {
		Testable testable;
		try {
			testable = TestTree.fromFile(new File(
					"C:\\Users\\mcaly\\eclipse-workspace\\CITS2200_Project_2022-release\\Subsidiaries\\testdata"));
		} catch (Throwable e) {
		...
		testable.buildReport(builder, "Testing Subsidiaries", 0);
